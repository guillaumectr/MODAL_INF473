...
def read_options(file, path="/users/eleves-b/2022/guillaume.coutiere/MODAL/MODAL_INF473/data/dataset_generators/options/", n=-1):
    ...
    def create_prompts(self, labels_names):
        ...
        for label in labels_names:
            ...
            for i in range(self.num_images_per_label):
                adje = random.choices(adj, adj_weigths, k=1)[0]
                natu = random.choices(nat, nat_weigths, k=1)[0]
                qnty = random.choices(qty, qty_weigths, k=1)[0]
                chadj = random.choices(cadj, cadj_weigths, k=1)[0]
                situ = random.choices(sit, sit_weigths, k=1)[0]
                prompts[label].append(
                                    {
                                        "prompt": f"{adje} {natu} of {qnty} {chadj} {label} cheese {situ}",
                                        "num_images": 1,
                                    } 
                )
        return prompts
